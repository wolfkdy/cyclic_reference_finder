...
#ifdef KDY_DEBUG
static PyObject *gc_collect_cb = NULL;
#endif
...
#ifdef KDY_DEBUG
    if(gc_collect_cb != NULL) {
	    PyObject* cb_lst = PyList_New(0);
	    PyObject* op = NULL;
	    for(gc = unreachable.gc.gc_next; gc != &unreachable;
			    gc = gc->gc.gc_next){
		op = FROM_GC(gc);
		PyList_Append(cb_lst, op);
            }
	    if(PySequence_Size(cb_lst) > 0){
		    PyObject * ret = PyObject_CallFunctionObjArgs(gc_collect_cb, cb_lst, NULL);
		    if (ret == NULL){
			    PyErr_PrintEx(0);
		    }
		    Py_XDECREF(ret);
...
#ifdef KDY_DEBUG
PyDoc_STRVAR(gc_set_collect_callback__doc__,
"set_collect_callback(obj) -> \n"
"\n"
"Returns a list of objects which are not reachable during gc collecting\n"
"Simple atomic objects will return false.\n"
);
static PyObject *
gc_set_collect_callback(PyObject * self, PyObject *args)
{
	PyObject * op = NULL;
	...
	{
		PyErr_SetString(PyExc_RuntimeError, "param must be callable");
		return NULL;
	}
	if(gc_collect_cb != NULL)
	{
		Py_DECREF(gc_collect_cb);
	}
	gc_collect_cb = op;
	Py_INCREF(gc_collect_cb);
	Py_RETURN_NONE;
}
#endif
...
#ifdef KDY_DEBUG
"set_collect_callback() -- Return a list of unreachable objects.\n"
#endif
);
...
#ifdef KDY_DEBUG
    {"set_collect_callback", gc_set_collect_callback, METH_VARARGS,
	gc_set_collect_callback__doc__},
#endif
    {NULL,      NULL}           /* Sentinel */
};
